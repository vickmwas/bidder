...
public class Node {
     ...
     * Not in the REGEX
     */
    public static final int IP_COMPARE = 22;
    ...
    public void setValues() throws Exception {
        ...
        if (value instanceof String) {
            ...
            sval = (String) value;
            logger.error("Retrieved string value from Target: {}", sval);
        }
     ...
     * @param value     Object. The value that the bid request specified by hierarchy
    ...
    public boolean test(BidRequest br, StringBuilder errorString) throws Exception {
        ...
        if (oldOperator == QUERY) {
            ...
            if (brValue != null) {
                if (operator == IP_COMPARE)
                    logger.warn("Operator: {}, Hierarchy: {}, BRValue = {}", OPNAMES.get(operator), hierarchy, brValue);
     ...
     *
     * @param value Object. Converts the value of the bid request field (Jackson)
    ...
    public boolean testInternal(Object value) throws Exception {
        ...
        if (value instanceof String) {
            svalue = (String) value;
            logger.warn("Retrieved String Value from BidRequest: {} => {}", hierarchy, svalue);
        }
...
//*****************************************************************************************************
            case IP_COMPARE:
                if (sval == null) {
                    logger.warn("sval from target: null");
                    return false;
                } else if (svalue == null) {
                    logger.warn("sval from bidrequest: null");
                    return false;
                } else {
                    logger.warn("Performing IP comparison : {} in {}", svalue, sval);
                    return computeIPComparison(sval, svalue);
                }
...
//*****************************************************************************************************
    ...
    /**
     * Compute IP Comparison
     *
     * @param sval   The List of IP Addresses from the Target stored in DB
     * @param svalue The Device IP From the Bid Request, as obtained through device.ip
     * @return boolean The result of the comparison
     */
    private boolean computeIPComparison(String sval, String svalue) {
        for (String ip_lookup : sval.split(",")) {
            if (truncateIP(svalue).equalsIgnoreCase(truncateIP(ip_lookup))) {
                return true;
            }
        }
    ...
    private String truncateIP(String originalIP) {
        return String.join(".", Arrays.copyOfRange(originalIP.split("\\."), 0, 2));
    }

//	public static void main(String[] args) {
//		String svalue = "154.57.9.0";
//		String sval = "154.52.0.0,154.53.0.0,154.57.0.0";
//
//		boolean found = false;
//		for (String ip_lookup : sval.split(",")){
//			if(truncateIP(svalue).equalsIgnoreCase(truncateIP(ip_lookup))){
...
//		if(pos == null){
//			logger.error("device.geo is null");
//			return false;
//		} else{
        double ylat = pos.get("lat");
        double ylon = pos.get("lon");
        ...
        for (int i = 0; i < qvalue.size(); i++) {
            ...
            if (range < limit) {
                logger.error("lat {},lon {} from BidRequest is within range {} of lat {}, lon {} of DB",
                        ylat, ylon, limit, xlat, xlon);
                return true;
            }
        }

        logger.error("lat {},lon {} from BidRequest not in Range",
                ylat, ylon);
        logger.error("=====================================================");
        return false;
//		}
